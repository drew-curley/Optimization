...
def load_and_preprocess(filepath):
    ...
    df['gender_code'] = (df['gender'].str.lower() == 'm').astype(int)  # 1=male, 0=female
...
def standardize_predictors(df):
    ...
    continuous_vars = ['recency', 'frequency', 'tenure', 'capacity_score', 'age']
...
def build_model(df):
    ...
    with pm.Model() as donor_model:
        ...
        alpha_gamma = pm.Normal('alpha_gamma', mu=8, sigma=2)  # log scale (~$3000)
        ...
        expected_value = pm.Deterministic('expected_value', p_give * mu_gift_all)
...
def get_expected_values(trace, df, top_n=50):
    ...
    p_give_samples = trace.posterior['p_give'].values.reshape(n_chains * n_draws, n_donors)
    mu_gift_samples = trace.posterior['mu_gift_all'].values.reshape(n_chains * n_draws, n_donors)
...
if __name__ == "__main__":
    ...
    # Load data
    print("Loading data...")
    df = load_and_preprocess('/mnt/user-data/uploads/donors.csv')
    df, means, stds = standardize_predictors(df)
    
    print(f"Loaded {len(df)} donors")
    print(f"Donors who gave: {df['gave'].sum()}")
    ...
    # Save full results
    top_donors.to_csv('/mnt/user-data/outputs/top_50_donors.csv', index=False)
    print("\nResults saved to /mnt/user-data/outputs/top_50_donors.csv")
    
    # Save trace for later analysis
    trace.to_netcdf('/mnt/user-data/outputs/donor_trace.nc')
    print("MCMC trace saved to /mnt/user-data/outputs/donor_trace.nc")
